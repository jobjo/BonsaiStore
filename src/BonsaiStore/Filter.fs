...
module Filter =
    ...
    module TV = FSharp.BonsaiStore.Internal.Traverse
...
//                let cs = string <| box c
//                TP.print <| sprintf "%s[%s,%s]" fts ops cs
//            | Not f             -> 
//                block "Not" (go f)
//            | And (f1,f2)       -> 
//                block "And" !<[go f1; go f2]
//            | Or (f1,f2)        -> 
//                block "Or" !<[go f1; go f2]
//        go >> TP.run
//        
    ...
    let find f t = TV.find (toPath f) t

    /// Given a identity, a mapper and reducer function traverses the
    let mapReduce mzero map reduce f t = 
        TV.mapReduce mzero map reduce (toPath f) t